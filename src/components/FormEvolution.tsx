          ...
          <Card.Body>
            <div className="d-flex gap-3">
              <img
                ...
                src={instancia.instance.profilePictureUrl || qrCode || imagemSemConexao}
                width={200}
              />
              <div>
                ...
                {instancia.instance.status === 'close' ? (
                  <>
                    <p className="text-danger">
                      <FontAwesomeIcon icon={faCircleXmark} /> Inativo
                    </p>
                    <p className="fst-italic opacity-75 mb-0">
                      Ative a conexão clicando no botão "Ligar" abaixo
                    </p>
                  </>
                ) : instancia.instance.status === 'connecting' ? (
                  <p className="text-warning">
                    <FontAwesomeIcon icon={faClock} /> Conectando
                  </p>
                ) : (
                  <>
                    <p>
                      <FontAwesomeIcon icon={faMobileScreenButton} />{' '}
                      {instancia.instance.owner.split('@')[0]}
                    </p>
                    <p>
                      <FontAwesomeIcon icon={faUser} /> {instancia.instance.profileName}
                    </p>
                    <p className="text-success">
                      <FontAwesomeIcon icon={faCircleCheck} /> Ativo
                    </p>
                    {!carregandoWebhook ? (
                      <Form.Check
                        type="switch"
            ...
            <div className="d-flex mt-2 gap-2">
              ...
              <Button
                className={instancia.instance.status === 'close' ? 'btn-success' : 'btn-danger'}
                onClick={() => ligarDesligarInstancia()}
                disabled={carregandoInstancia}
              >
