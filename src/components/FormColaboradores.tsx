  ...
  return (
    <Form>
      ...
      <p className="fst-italic opacity-75">
        Os colaboradores listados abaixo serão reconhecidos pelos seus assistentes de IA.
      </p>
      <Form.Group className="mb-3">
        <Form.Label>Colaboradores (selecione um colaborador para modificá-lo)</Form.Label>
        <Form.Select
          value={colabSelecionado?.id || colabSelecionado}
          onChange={(opcao) => alterarColabSelecionado(opcao.target.value)}
        >
          <option>--</option>
      ...
      {colabSelecionado ? (
        <Accordion className="pb-3">
          <Accordion.Item eventKey="0">
            ...
            <Accordion.Body>
              <FormColaboradorUnico
                colaborador={colabSelecionado}
                selecionar={(colab) => setColabSelecionado(colab)}
              />
            </Accordion.Body>
          </Accordion.Item>
        </Accordion>
