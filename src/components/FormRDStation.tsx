  ...
  return (
    <Form>
      <Form.Group className="mb-3">
        ...
        <Form.Control
          type="text"
          placeholder="Token do RD Station CRM"
          value={token}
      ...
      <Form.Group className="mb-3">
        ...
        <Form.Control
          type="text"
          placeholder="ID da fonte padrão da lead"
          value={idFontePadrao}
          onChange={(e) => setIdFontePadrao(e.target.value)}
        />
      </Form.Group>
      <Form.Group className="mb-3">
        ...
        <Form.Select onChange={(opcao) => alterarEstagioSelecionado(opcao.target.value)}>
          ...
          {estagios
            ? estagios.map((estagio) => <option value={estagio.id}>{estagio.atalho}</option>)
            : ''}
        </Form.Select>
      </Form.Group>
      {estagioSelecionado ? (
        <Accordion className="pb-3">
          <Accordion.Item eventKey="0">
            <Accordion.Header>Estágio {estagioSelecionado.id}</Accordion.Header>
            <Accordion.Body>
              <FormEstagioRD
                estagio={estagioSelecionado}
                selecionar={(est) => setEstagioSelecionado(est)}
              />
            </Accordion.Body>
          </Accordion.Item>
        </Accordion>
