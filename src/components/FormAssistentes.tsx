    ...
    <Form>
      ...
      <Form.Group className="mb-3">
        ...
        <p className="fst-italic opacity-75">
          Apenas assistentes de propósito "responder" podem ser selecionados como assistente padrão.
        </p>
      ...
      {assSelecionado ? (
        <Accordion className="pb-3">
          <Accordion.Item eventKey="0">
            ...
            <Accordion.Body>
              <FormAssistenteUnico
                assistente={assSelecionado}
                selecionar={(ass) => setAssSelecionado(ass)}
              />
            </Accordion.Body>
          </Accordion.Item>
        </Accordion>
      ...
      <Form.Group className="mb-3">
        ...
        <Form.Select onChange={(opcao) => alterarVozSelecionada(opcao.target.value)}>
          ...
          {vozes ? vozes.map((voz) => <option value={voz.id}>{voz.nome}</option>) : ''}
        </Form.Select>
      </Form.Group>
      {vozSelecionada ? (
        <Accordion className="pb-3">
          <Accordion.Item eventKey="0">
            <Accordion.Header>
              {vozSelecionada !== '+' ? `Voz ${vozSelecionada.nome}` : 'Nova voz'}
            </Accordion.Header>
            <Accordion.Body>
