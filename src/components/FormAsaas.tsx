    ...
    <Form>
      ...
      <Form.Group className="mb-3">
        ...
        <Form.Select onChange={(opcao) => alterarClienteSelecionado(opcao.target.value)}>
          ...
          {asaasClients
            ? asaasClients.map((cliente) => <option value={cliente.id}>{cliente.rotulo}</option>)
            : ''}
        </Form.Select>
      </Form.Group>
      {clienteSelecionado ? (
        <Accordion className="pb-3">
          <Accordion.Item eventKey="0">
            <Accordion.Header>
              {clienteSelecionado !== '+' ? `Cliente ${clienteSelecionado.rotulo}` : 'Novo cliente'}
            </Accordion.Header>
            <Accordion.Body>
              <FormAsaasUnico
                asaasClient={clienteSelecionado}
                selecionar={(cli) => setClienteSelecionado(cli)}
              />
            </Accordion.Body>
          </Accordion.Item>
        </Accordion>
