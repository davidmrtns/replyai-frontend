...
function FormColaboradorUnico({ colaborador, selecionar }) {
    ...
    setEmpresa((prevEmpresa) => {
      return {
        ...
        colaboradores: prevEmpresa.colaboradores.map((colaborador) =>
          colaborador.id === id ? { ...colaborador, ...colabAtualizado } : colaborador,
        ),
      }
    })
  }
  ...
  const delColaborador = (id) => {
    setEmpresa((prevEmpresa) => {
      return {
        ...prevEmpresa,
        colaboradores: prevEmpresa.colaboradores.filter((colaborador) => colaborador.id !== id),
      }
    })
  ...
  const enviar = async () => {
    ...
    if (colaborador === '+') {
      ...
      var resposta = await apiFetch.alterarColaborador(
        empresa.slug,
        colaborador.id,
        nome,
        apelido,
    ...
    <Form>
      ...
      <Form.Group className="mb-3">
        ...
        <p className="fst-italic opacity-75">
          Preencha esse campo caso os seus clientes se refiram a esse colaborador por algum apelido.
        </p>
        <Form.Control
          type="text"
          placeholder="Apelido do colaborador (opcional)"
          value={apelido}
