  ...
  return (
    <Form>
      ...
      <Form.Group className="mb-3">
        ...
        <Form.Control
          type="text"
          placeholder="Atalho do assistente"
          value={atalho}
          onChange={(e) => setAtalho(e.target.value.toUpperCase().trim())}
        />
      </Form.Group>
      <Form.Group className="mb-3">
        {carregandoInstrucoes ? (
          ...
          <>
            ...
            {proposito ? (
              <>
                <p className="fst-italic opacity-75">
                  Abaixo, você encontrará um exemplo de prompt adequado ao propósito escolhido para
                  o assistente. Cada assistente segue um formato específico de prompt e resposta,
                  conforme seu propósito – você pode conferir esses padrões no menu abaixo. Se
                  desejar, copie o prompt para a caixa de instruções clicando no botão abaixo e, em
                  seguida, personalize os campos necessários para atender às necessidades do seu
                  assistente. Se você não tiver muita experiência com a criação de prompts,
                  experimente utilizar os exemplos substituindo apenas os valores dentro dos
                  colchetes [ ].
                </p>
                ...
                <Accordion className="pb-3">
                  <Accordion.Item eventKey="0">
                    <Accordion.Header>Exemplo de prompt - {proposito}</Accordion.Header>
                    <Accordion.Body>
                      {carregandoPromptExemplo ? (
                        <p className="mb-0">
                          ...
                          Carregando prompt de exemplo
                        </p>
                      ) : (
                        <ReactMarkdown remarkPlugins={[remarkGfm]}>
                          {promptExemplo || 'Nenhum exemplo de prompt encontrado...'}
                        </ReactMarkdown>
                      )}
                    </Accordion.Body>
                  </Accordion.Item>
      ...
      <Form.Group className="mb-3">
        ...
        <Form.Select value={idVoz} onChange={(opcao) => setIdVoz(opcao.target.value)}>
          <option>--</option>
          {empresa.vozes
            ? empresa.vozes.map((voz) => <option value={voz.id}>{voz.nome}</option>)
            : ''}
        </Form.Select>
      </Form.Group>
