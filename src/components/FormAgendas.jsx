  ...
  return (
    <Form>
      ...
      <Form.Group className="mb-3">
        ...
        <Form.Select
          value={tipoAgendaClient}
          onChange={(opcao) => setTipoAgendaClient(opcao.target.value)}
        >
          <option value="">--</option>
      ...
      <Form.Group className="mb-3">
        ...
        <Form.Control
          ...
          min="00:00:00"
          max="23:59:59"
          value={horaFinalAgenda}
          onChange={(e) => setHoraFinalAgenda(e.target.value)}
        />
      </Form.Group>
      <hr />
      <p className="fst-italic opacity-75">
        As agendas listadas abaixo são as agendas em que as IAs poderão adicionar agendamentos, e
        também as agendas que terão os seus eventos confirmados (se ativado).
      </p>
      <Form.Group className="mb-3">
        ...
        <Form.Select onChange={(opcao) => alterarAgendaSelecionada(opcao.target.value)}>
          ...
          {agendas
            ? agendas.map((agenda) => <option value={agenda.id}>{agenda.endereco}</option>)
            : ''}
        </Form.Select>
      </Form.Group>
      {agendaSelecionada ? (
        <Accordion className="pb-3">
          <Accordion.Item eventKey="0">
            <Accordion.Header>
              {agendaSelecionada !== '+' ? `Agenda ${agendaSelecionada.endereco}` : 'Nova agenda'}
            </Accordion.Header>
            <Accordion.Body>
              <FormAgendaUnica
                agenda={agendaSelecionada}
                selecionar={(ag) => setAgendaSelecionada(ag)}
              />
            </Accordion.Body>
          </Accordion.Item>
        </Accordion>
      ...
      {tipoAgendaClient ? (
        <Accordion className="pb-3">
          <Accordion.Item eventKey="0">
            <Accordion.Header>
              {tipoAgendaClient == 'outlook'
                ? 'Configurações do Outlook'
                : 'Configurações do Google Agenda'}
            </Accordion.Header>
            <Accordion.Body>
              {tipoAgendaClient == 'outlook' ? <FormOutlook /> : <FormGoogleCalendar />}
            </Accordion.Body>
          </Accordion.Item>
        </Accordion>
      ) : (
