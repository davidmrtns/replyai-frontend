        ...
        {usuarioLogado.admin ? (
          <>
            <Form className="d-flex flex-row align-items-center justify-content-center w-100 gap-2 pb-4">
              ...
              <Button
                disabled={!usuarios?.has_more}
                onClick={() => setCursor(usuarios?.next_cursor)}
              >
                    ...
                    usuarios.data.map((usuario) => (
                      <Col key={usuario.id} xs={12} sm={6} md={4} lg={3}>
                        <Card style={{ width: '100%' }}>
                          <Card.Header style={{ fontSize: '0.92em' }}>{usuario.nome}</Card.Header>
                          <Card.Body>
                            <Card.Subtitle>{usuario.email}</Card.Subtitle>
                            <p className="mb-0">
                              {usuario.admin ? 'Administrador' : 'Usuário padrão'}
                            </p>
                            <Button
                              variant="primary"
                              className="mt-2"
                              onClick={() => acessarUsuario(usuario)}
                            >
                              Ver usuário
                            </Button>
                          </Card.Body>
