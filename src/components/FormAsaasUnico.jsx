...
function FormAsaasUnico({ asaasClient, selecionar }) {
  ...
  const enviar = async () => {
    ...
    if (asaasClient === '+') {
      ...
      var resposta = await apiFetch.editarInformacoesClienteAsaas(
        empresa.slug,
        token,
        rotulo,
        clientNumber,
  ...
  return (
    <Form>
      ...
      <Form.Group className="mb-3">
        ...
        <Form.Control
          type="text"
          placeholder="Rótulo da conta"
          value={rotulo}
          onChange={(e) => setRotulo(e.target.value.toUpperCase().trim())}
        />
      </Form.Group>
      <Form.Group className="mb-3">
        ...
        <Form.Control
          ...
          min="0"
          value={clientNumber}
          disabled={asaasClient !== '+'}
          onChange={(e) => setClientNumer(e.target.value)}
        />
      </Form.Group>
      {asaasClient !== '+' ? (
        <>
