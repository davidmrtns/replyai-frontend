...
function FormMidiaUnica({ midia, selecionar }) {
    ...
    setEmpresa((prevEmpresa) => {
      return {
        ...
        midias: prevEmpresa.midias.map((midia) =>
          midia.id === id ? { ...midia, ...midiaAtualizada } : midia,
        ),
      }
    })
  }
  ...
  const delMidia = (id) => {
    setEmpresa((prevEmpresa) => {
      return {
        ...prevEmpresa,
        midias: prevEmpresa.midias.filter((midia) => midia.id !== id),
      }
    })
    ...
    <Form>
          ...
          <div className="d-flex flex-column gap-2">
            {mediatype?.startsWith('image/') ? (
              ...
              <p className="fst-italic opacity-75 mb-0">
                Esse arquivo não tem uma visualização.{' '}
      ...
      <Form.Group className="mb-3">
        ...
        <p className="fst-italic opacity-75">
          As IAs poderão enviar a sua mídia utilizando este atalho. Caso queira enviar vários
          arquivos no mesmo comando, insira um atalho já existente.
        </p>
        <Form.Control
          type="text"
          placeholder="Atalho"
      ...
      <Form.Group className="mb-3">
        ...
        <p className="fst-italic opacity-75">
          A ordem é utilizada para envios de múltiplos arquivos de mesmo código de atalho.
        </p>
        <Form.Control
          type="number"
          placeholder="Ordem do arquivo"
          value={ordem}
