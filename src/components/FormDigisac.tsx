      ...
      <Form>
        <Form.Group className="mb-3">
          ...
          <p className="fst-italic opacity-75">
            Parte da URL de acesso do Digisac que vem depois de <strong>https://</strong> e antes de{' '}
            <strong>.digisac.me</strong>
          </p>
          <Form.Control
            type="text"
            placeholder="Slug do Digisac"
            value={slugDigisac}
            onChange={(e) => setSlugDigisac(e.target.value)}
          />
        </Form.Group>
        <Form.Group className="mb-3">
          <Form.Label>Token do Digisac</Form.Label>
          <p className="fst-italic opacity-75">
            Insira o token de integração obtido do Digisac em Conta {`>`} API {`>`} Tokens de acesso
            pessoal
          </p>
          <Form.Control
            type="text"
            placeholder="Token do Digisac"
            value={tokenDigisac}
            onChange={(e) => setTokenDigisac(e.target.value)}
          />
        </Form.Group>
        {digisacClient ? (
          <>
            <Form.Group className="mb-3">
              ...
              <p className="fst-italic opacity-75">
                Usuário responsável por realizar as transferências automatizadas entre departamentos
              </p>
              <AsyncSelect
                cacheOptions
                defaultOptions
                ...
                noOptionsMessage={() =>
                  'Nenhum resultado encontrado. Digite para fazer uma pesquisa.'
                }
            ...
            <hr />
            <p className="fst-italic opacity-75">
              Os departamentos listados abaixo referem-se aos departamentos que você cadastrou e
              para os quais as IAs podem fazer transferências. Pode ser que nem todos os
              departamentos do seu Digisac estejam listados aqui.
            </p>
            <Form.Group className="mb-3">
            ...
            {deptSelecionado ? (
              <Accordion className="pb-3">
                <Accordion.Item eventKey="0">
                  <Accordion.Header>Departamento {deptSelecionado.atalho}</Accordion.Header>
                  <Accordion.Body>
                    <FormDeptDigisac
                      departamento={deptSelecionado}
                      selecionar={(dpt) => setDeptSelecionado(dpt)}
                    />
                  </Accordion.Body>
                </Accordion.Item>
              </Accordion>
